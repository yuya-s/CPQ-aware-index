    ...
    for(int for_k = 1;for_k<k;for_k++) {// for all k length path
    	...
    	if(for_k==1){
            ...
            for (Segment segment : seg_prefix[0]) {
                count_enumeration++;
		//if(count_enumeration%100==0)cout<<count_enumeration<<","<<seg[for_k].size()<<endl;
		    ...
		    int pathid;
		    if(pathlist[pathkey.src].find(pathkey.dst)==pathlist[pathkey.src].end())pathid=0;
                    else pathid = pathlist[pathkey.src][pathkey.dst];
		    //cout<<pathlist[pathkey.src].size()<<endl;
		    //pathlist[pathkey.src].set_deleted_key(pathkey.dst);
		    //pathlist[pathkey.src].erase(pathkey.dst);
		    //pathlist[pathkey.src].clear_deleted_key();
                    ...
                    if(pathid == 0) {
                        ...
                        for (auto label_l : segment.labelsequence) {
                            for (auto label_0 : seg[0][id_path0].labelsequence) {
                                long int edgelabel = (1 + label_l) * number_of_labels + label_0;

                               if (workload.prefixlabelidhash[edgelabel]==1||workload.labelidhash[edgelabel]==1)tempSegment.labelsequence.insert(edgelabel);
                        ...
                        for (auto label_l : segment.labelsequence) {
                            for (auto label_0 : seg[0][id_path0].labelsequence) {
                                long int edgelabel = (1 + label_l) * number_of_labels + label_0;
                                if (workload.prefixlabelidhash[edgelabel]==1||workload.labelidhash[edgelabel]==1)seg[for_k][pathid-1].labelsequence.insert(edgelabel);
                            }
                        }
                    }
                }
   ...
   for(int for_k=1;for_k <k ;for_k++){
	for(Segment segment : seg[for_k]){	
            ...
            if(pathid == 0) {
                ...
                for (auto label_l : segment.labelsequence) {
                        if (workload.labelidhash[label_l]==1)tempSegment.labelsequence.insert(label_l);
                    }
                ...
                if(!tempSegment.labelsequence.empty()){
                    ...
                    for (auto label_l : segment.labelsequence) {
                            if (workload.labelidhash[label_l]==1)seg[0][pathid-1].labelsequence.insert(label_l);
                        }
                    }
		}
   }
