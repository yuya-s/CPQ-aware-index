...
inline bool segmentcmppointer(const Segment *s1, const Segment *s2){
	if(s1->labelsequence.size() == s2->labelsequence.size()){
		if(s1->labelsequence.size()==0)cout<<"bug"<<endl;
...
inline vector<int> decodeLabel(long int labelsequence, int labelnum, int k){
    ...
    while(1){
        if(labelsequence/(int)pow(labelnum,k_value)>0) {
            labellist.push_back((int)(labelsequence % (int) pow(labelnum, k_value)-1));
            labelsequence = labelsequence/(int)pow(labelnum,k_value);
        }
        k_value--;
        if(k_value==0){
            labellist.push_back((int)labelsequence);
            break;
        }
    }
    return labellist;
...
inline long int encodeLabel(vector<int>& labellist, int labelnum, int k){
