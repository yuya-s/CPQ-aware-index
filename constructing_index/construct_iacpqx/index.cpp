...
PathIndex::PathIndex(WorkloadawareTransequivalent& k_pathset){
    ...
    labelidhash_index.set_empty_key(-1);
	...
	if(previousSegment == NULL){
		...
		for(auto l : seg->labelsequence){
            if(labelidhash_index[l]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(history2path.size());
                    label2history.push_back(temphistory);
                    labelidhash_index[l]=label2history.size();
                    indexhash_labelid[label2history.size()]=l;
                }
            else {label2history[labelidhash_index[l]-1].push_back(history2path.size());}
