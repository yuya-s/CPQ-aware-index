...
inline bool cmpsegvaluepointer(const Segment *a, const Segment *b)
{
    if(a->possiblitilyofbisimilar&&!b->possiblitilyofbisimilar)return true;
        ...
        if(a->segset.size() == b->segset.size()) {
            ...
            if (a->path.src == a->path.dst&& b->path.src == b->path.dst)return a->path.src <= b->path.src;
            else if (a->path.src == a->path.dst)return true;
            else if (b->path.src == b->path.dst)return false;
            else if (a->path.src < b->path.src)return true;
            else if (a->path.src == b->path.src && a->path.dst <= b->path.dst)return true;
            return false;
...
//inline vector<int> decodeLabel(int labelidset, int labelnum, int k){
//
//    vector<int> labellist;
//    int k_value=k;
//
//    while(1){
//        if(labelidset/(int)pow(labelnum,k_value)>0) {
//            labellist.push_back(labelidset % (int) pow(labelnum, k_value)-1);
//            labelidset = labelidset/(int)pow(labelnum,k_value);
//        }
//        k_value--;
//        if(k_value==0){
//            labellist.push_back(labelidset);
//            break;
//        }
//    }
//    return labellist;
...
//
//        if(i==0)labelidset += labellist[i];
//        else{
//            labelidset += (labellist[i]+1)*labelnum;
//        }
//    }
//    return labelidset;
//}
