        ...
        for (auto segment : k_pathbisimulation.seg[for_k]) {
            ...
            for (auto label: segment.labelidset) {
                if(hash_labelid2index[label]==0){
                    ...
                    temphistory.clear();
                    temphistory.push_back(historyid-1);
                    label2history.push_back(temphistory);
                    hash_labelid2index[label]=label2history.size();
                    hash_index2labelid[label2history.size()]=label;
                }
            else {label2history[hash_labelid2index[label]-1].push_back(historyid-1);}
...
//        cout<<hash_index2labelid[labelid+1]<<" "<<l2h.size()<<endl;
//        labelid++;
//    }
