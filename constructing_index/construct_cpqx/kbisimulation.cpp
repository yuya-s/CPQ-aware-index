...
KPathBisimulation::KPathBisimulation(int k_ , OriginalGraph graph_) {
    ...
    for(auto edge : graph_.edge_list){
        ...
        if(pathid == 0) {
            ...
            tempPath.labelidset.insert(edge.label);
            pathset.push_back(tempPath);
            pathlist[pathkey.src][pathkey.dst]=pathset.size();
        }
        else{
            pathset[pathid-1].labelidset.insert(edge.label);
        }
        ...
        if(pathid==0) {
            tempPath.path = pathkey;
            tempPath.labelidset.insert(edge.label + graph_.number_of_labels);
            pathset.push_back(tempPath);
            pathlist[pathkey.src][pathkey.dst]=pathset.size();
        }
        else{
            pathset[pathid-1].labelidset.insert(edge.label + graph_.number_of_labels);
        }
    }
    ...
    for(auto s : pathset){
        ...
        vector<long int> vec(s.labelidset.begin(), s.labelidset.end());
        tempSegment.labelidset = vec;
        tempSegment.order = seg[0].size();
        tempSegment.segvalue = hash_set(s.labelidset);
            ...
            for (auto endVertex : pathlist0[segment.path.dst]) {
                ...
                if(pathid == 0) {
                     ...
                     for (auto labelid : segment.labelidset) {
                         for (auto label_0 : seg0.labelidset) {
                             tempPath.labelidset.insert((1 + labelid) * number_of_labels + label_0);
                         }
                     }
                     pathset.push_back(tempPath);
                     pathlist[pathkey.src][pathkey.dst]=pathset.size();
                   ...
                   for (auto labelid : segment.labelidset) {
                         for (auto label_0 : seg0.labelidset) {
                             pathset[pathid-1].labelidset.insert((1 + labelid) * number_of_labels + label_0);
                         }
                     }
