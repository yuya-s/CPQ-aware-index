...
WorkloadawareTransequivalentIndex::WorkloadawareTransequivalentIndex(WorkloadawareTransequivalent& k_pathset){
	...
	if(previousSegment == NULL){
		...
		for(auto l : seg->labelidset){
            if(hash_labelid2index[l]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(history2path.size());
                    label2history.push_back(temphistory);
                    hash_labelid2index[l]=label2history.size();
                    hash_index2labelid[label2history.size()]=l;
                }
            else {label2history[hash_labelid2index[l]-1].push_back(history2path.size());}

		}
	}
	else if(seg->labelidset == previousSegment->labelidset&&loopFlag==previousLoopFlag){
		te_pathset.push_back(seg->path);	
	}	
	else{
...
void WorkloadawareTransequivalentIndex::Output(string indexfilename){
