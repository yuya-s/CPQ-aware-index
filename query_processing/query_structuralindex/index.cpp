...
BisimulationIndex::BisimulationIndex(std::string input_file_name)
{
    ...
    while(std::getline(inputl2h,line)){
        ...
        hash_labelid2index[labelId]=label2history.size();
        hash_index2labelid[label2history.size()]=labelId;
    }
...
void BisimulationIndex::SetWorkload(std::string workload_file_name){
    ...
    while(std::getline(inputwork,line)) {
        ...
        if (hash_labelid2index[labelsequence] == 0){
            ...
            temphistory.clear();
            label2history.push_back(temphistory);
            hash_labelid2index[labelsequence] = label2history.size();
        }
...
void BisimulationIndex::DeleteEdge(Edge dedge, OriginalGraph& graph)
{
    ...
    vector<vector<bool>> kth_labels_workloadload;
    kth_labels_workloadload.resize(k);
    for(int for_k=0;for_k<k;for_k++) {
        kth_labels_workloadload[for_k].resize(labelnum);
        for (int i = 0; i < labelnum; i++) {
            kth_labels_workloadload[for_k][i]=false;
        }
    }
    ...
    for (int for_k = 1; for_k < k; for_k++) {
        ...
        while (itr_de != deletedPathList[for_k].end()) {
            if (workloadindex&&hash_labelid2index[(*itr_de).label]==0) {
                ...
                for(int i=0;i<templabelsequence.size();i++){
                    kth_labels_workloadload[templabelsequence.size()-i-1][templabelsequence[i]]=true;
    ...
    for(int for_k=1;for_k< k; for_k++) {
        ...
        while (itr_de != deletedPathList[for_k].end()) {
            ...
            for (auto edgeid : graph.vertex_edge_list[deletedPath.src]) {
                if (!kth_labels_workloadload[0][graph.edge_list[edgeid].label]||graph.edge_list[edgeid].label == -1)continue;
                if (dedgeID != edgeid) {
            ...
            for (auto edgeid : graph.vertex_inverseedge_list[deletedPath.src]) {
                if (!kth_labels_workloadload[0][graph.edge_list[edgeid].label+graph.number_of_labels]||graph.edge_list[edgeid].label == -1)continue;
                if (dedgeID != edgeid) {
            ...
            while (1) { // BFS
                ...
                for (auto edge : candidatePathList) {
                    for (auto edgeid : graph.vertex_edge_list[edge.dst]) {
                        if (!kth_labels_workloadload[count][graph.edge_list[edgeid].label]||graph.edge_list[edgeid].label == -1)continue;
                    ...
                    for (auto edgeid : graph.vertex_inverseedge_list[edge.dst]) {
                        if (!kth_labels_workloadload[count][graph.edge_list[edgeid].label+graph.number_of_labels]||graph.edge_list[edgeid].label == -1)continue;
                        //if(count==for_k && !dstcheck[graph.edge_list[edgeid].dst])continue;
                        if (dedgeID != edgeid) {
                            int labelsequnce = (edge.label + 1) * labelnum + graph.edge_list[edgeid].label +
            ...
            for (auto edgeid : graph.vertex_edge_list[deletedPath.dst]) {
                if (!kth_labels_workloadload[for_k][graph.edge_list[edgeid].label+graph.number_of_labels]||graph.edge_list[edgeid].label == -1)continue;
                if (dedgeID != edgeid) {
            ...
            for (auto edgeid : graph.vertex_inverseedge_list[deletedPath.dst]) {
                if (!kth_labels_workloadload[for_k][graph.edge_list[edgeid].label]||graph.edge_list[edgeid].label == -1)continue;
                if (dedgeID != edgeid) {
            ...
            while(left<=right){
                ...
                if(label2history[for_l][for_h]==history_of_dpath){
                    ...
                    for(int i=0;i<dPath.labelset.size();i++){
                        if(hash_labelid2index[dPath.labelset[i]]-1==for_l){sameLabelFlag=true;break;}
                    }
                    if(!sameLabelFlag){
    ...
    for(auto aPath : addedPathWLabelset){
        ...
        if(for_h!=-1){
            ...
            for(int i=0;i<aPath.labelset.size();i++){
                if(hash_labelid2index[aPath.labelset[i]]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(newhistory);
                    label2history.push_back(temphistory);
                    hash_labelid2index[aPath.labelset[i]]=label2history.size();
                }
                else {label2history[hash_labelid2index[aPath.labelset[i]]-1].push_back(newhistory);}
            }
            for (int for_l = 0; for_l < label2history.size(); for_l++) {
                if(label2history[for_l].empty())continue;
                ...
                while(left<=right){
                    ...
                    if(label2history[for_l][for_h]==history_of_apath){
                        ...
                        bool sameLabelFlag=false;
                        for(int i=0;i<aPath.labelset.size();i++){
                            if(hash_labelid2index[aPath.labelset[i]]-1==for_l){sameLabelFlag=true;break;}
                        }
            ...
            for(auto for_label : aPath.labelset){
                 if(hash_labelid2index[for_label]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(newhistory);
                    label2history.push_back(temphistory);
                    hash_labelid2index[for_label]=label2history.size();
                }
                else {label2history[hash_labelid2index[for_label]-1].push_back(newhistory);}
            }
...
void BisimulationIndex::DeleteWorkload(vector<int> labelsequences_, OriginalGraph& graph_) {
    ...
    if(!givenindexworkload[labelid])return;
    label2history[hash_labelid2index[labelid]-1].clear();
    hash_labelid2index[labelid] = 0;
    givenindexworkload[labelid] = false;
}
...
void BisimulationIndex::AddWorkload(vector<int> labelsequence_, OriginalGraph& graph_) {
    ...
    long int updatedlabelid = encodeLabel(labelsequence_, labelnum, k);
    if(givenindexworkload[updatedlabelid])return;
    givenindexworkload[updatedlabelid]=true;

    vector<long int> labelidset;

    long int prefixupdatedlabelid=labelsequence_[0];
    labelidset.push_back(prefixupdatedlabelid);
    ...
    for(int i=1;i<labelsequence_.size();i++){
        prefixupdatedlabelid = (prefixupdatedlabelid+1)*labelnum+labelsequence_[i];
        labelidset.push_back(prefixupdatedlabelid);
        labelidhash_addworkload[prefixupdatedlabelid]=1;
    }
                    ...
                    if (pathid == 0) {
                        ...
                        for (auto labelid : segment.labelidset) {
                            for (auto label_0 : seg[0][id_path0].labelidset) {
                                long int newlabelid = (1 + labelid) * labelnum + label_0;
                                if (labelidhash_addworkload[newlabelid]>0)tempSegment.labelidset.insert(newlabelid);
                            }
                        }
                        ...
                        for (auto labelid : segment.labelidset) {
                            for (auto label_0 : seg[0][id_path0].labelidset) {
                                long int newlabelid = (1 + labelid) * labelnum + label_0;
                                if (labelidhash_addworkload[newlabelid]>0)seg[for_k][pathid-1].labelidset.insert(newlabelid);
                            }
                        }
                    }
                }
...
//
        ...
        if(pathhistoryid[segcount]!= -1) {
            ...
            while (left <= right) {
                for_path = (left + right) / 2;
                if (history2path[for_h][for_path].first == aPath.src && history2path[for_h][for_path].second == aPath.dst) {
                    history2path[for_h].erase(history2path[for_h].begin() + for_path);

                    break;
                } else if (history2path[for_h][for_path].first < aPath.src || (history2path[for_h][for_path].first == aPath.src && history2path[for_h][for_path].second < aPath.dst)) {
                    left = for_path + 1;
                } else {
                    right = for_path - 1;
                }
        ...
        if(pathhistoryid[segcount]!= -1) {// if there is  same paths
            ...
            if(hash_labelid2index[updatedlabelid]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(newhistory);
                    label2history.push_back(temphistory);
                    hash_labelid2index[updatedlabelid]=label2history.size();
                    hash_index2labelid[label2history.size()]=updatedlabelid;
                }
            else {label2history[hash_labelid2index[updatedlabelid]-1].push_back(newhistory);}
            ...
            else {label2history[hash_labelid2index[updatedlabelid]-1].push_back(history2path.size());}
...
void BisimulationIndex::Output(string indexfilename){
    ...
    string outputL2H = indexfilename+"_l2h";
    string outputH2P = indexfilename+"_h2p";
    string outputWorkload = indexfilename+"_workload";
    ...
    for(int i=0; i<label2history.size();i++) {
        ...
        //cout<<hash_index2labelid[i+1]<<","<<label2history[i].size()<<endl;
        if(label2history[i].empty())continue;
        fout_l2h<<hash_index2labelid[i+1]<<" ";
    ...
    if(workloadindex) {
        ...
        for (auto itr = givenindexworkload.begin(); itr != givenindexworkload.end(); ++itr) {
            if(itr->second) {
