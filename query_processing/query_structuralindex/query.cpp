...
void QuerySet::InputFile(string workload_file, BisimulationIndex index_){
    ...
    if(!inputwork){
        std::cout<<"error: cannot open givenindexworkload files"<<std::endl;
        exit(1);
    }
...
void Query::Evaluation(vector<pair<int,int>>& answers, BisimulationIndex& bisimulationindex){
	...
	if(rootquery.hight==0){
        ...
        else Label2Path(answers, bisimulationindex,rootquery.labelsequence);
    }
    else{
    ...
    if(lpath.empty()&&rpath.empty()) {
        ...
        while (1) {
            ...
            if(l_Paths[lcount] == rpath[rcount]){
                answers.push_back(l_Paths[lcount]);
                rcount++;
                lcount++;
            }
...
void Query::Label2Path(vector<pair<int,int>>& answers,BisimulationIndex& bisimulationindex, vector<int>& labellist){
    ...
    //if(bisimulationindex.hash_labelid2index[labelid]==0)return;//for debug
    vector<int> historyset = bisimulationindex.label2history[bisimulationindex.hash_labelid2index[labelid]-1];
...
void Query::Label2History(vector<int>& histories, BisimulationIndex& bisimulationindex, vector<int>& labellist){
    ...
    long int labelid = encodeLabel(labellist, bisimulationindex.labelnum,bisimulationindex.k);
    //if(bisimulationindex.hash_labelid2index[labelid]==0)return;//for debug
    histories = bisimulationindex.label2history[bisimulationindex.hash_labelid2index[labelid]-1];
