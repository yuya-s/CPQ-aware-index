...
//        cout<<line<<endl;
        ...
        if(index_.workloadindex&&labelFlag){
            ...
            bool firstFlag;
            while(1) {
                if(index_.labelidhash[encodeLabel(labelset, index_.labelnum, index_.k)]>0){
...
void Query::Conjunction(BisimulationIndex& bisimulationindex, vector<pair<int,int>>& answers, vector<int>& histories, vector<pair<int,int>>& lpath, vector<pair<int,int>>& rpath, vector<int>& lhistory, vector<int>& rhistory){
	...
	lcount=0,rcount=0;
        while(1) {
	   // cout<<lcount<<","<<rcount<<","<<lhistory.size()<<","<<rhistory.size()<<endl;
        ...
        while (1) {
	    //cout<<lcount<<","<<rcount<<","<<lpath.size()<<","<<r_Paths.size()<<endl;
        ...
        while (1) {
	    //cout<<lcount<<","<<rcount<<","<<l_Paths.size()<<","<<rpath.size()<<endl;
            if(lcount>=l_Paths.size()||rcount>=rpath.size())break;
            if(l_Paths[lcount] == rpath[rcount]){
                answers.push_back(rpath[rcount]);
                rcount++;
                lcount++;
            }
        ...
        lcount=0,rcount=0;
        while (1) {
	    //cout<<lcount<<","<<rcount<<","<<lpath.size()<<","<<rpath.size()<<endl;
...
void Query::Label2Path(vector<pair<int,int>>& answers,BisimulationIndex& bisimulationindex, vector<int>& labellist){
    ...
    long int labelid = encodeLabel(labellist, bisimulationindex.labelnum,bisimulationindex.k);
    if(bisimulationindex.labelidhash.find(labelid)==bisimulationindex.labelidhash.end())return;
    vector<int> historyset = bisimulationindex.label2history[bisimulationindex.labelidhash[labelid]-1];
...
void Query::Label2History(vector<int>& histories, BisimulationIndex& bisimulationindex, vector<int>& labellist){
    ...
    long int labelid = encodeLabel(labellist, bisimulationindex.labelnum,bisimulationindex.k);
    //if(bisimulationindex.labelidhash[labelid]==0)return;
    histories = bisimulationindex.label2history[bisimulationindex.labelidhash[labelid]-1];
