...
BisimulationIndex::BisimulationIndex(std::string input_file_name)
{
    ...
    indexhash_labelid.set_empty_key(-1);
    ...
    while(std::getline(inputl2h,line)){
        ...
        vector<int> temphistory;
        temphistory.clear();
        while(ss >> history)
        {
            temphistory.push_back(history);
        }
        label2history.push_back(temphistory);
        labelidhash[labelId]=label2history.size();
        indexhash_labelid[label2history.size()]=labelId;
    }
    ...
    while(std::getline(inputwork,line)) {
        ...
        while (ss >> label) {
            if (std::all_of(label.cbegin(), label.cend(), [](char ch) { return isdigit(ch); })) {
                if (labelcount == 0) {
                    labelsequence = stol(label);
                } else {
                    labelsequence = (labelsequence + 1) * labelnum + stol(label);
                }
                labelcount++;
            }
        }
        if (labelidhash[labelsequence] == 0){
            vector<int> temphistory;
            temphistory.clear();
            label2history.push_back(temphistory);
            labelidhash[labelsequence] = label2history.size();
        }
        workload[labelsequence]=1;
    }
}
...
void BisimulationIndex::DeleteEdge(Edge dedge, OriginalGraph& graph)
{
    ...
    for (int for_k = 1; for_k < k; for_k++) {
        ...
        while (itr_de != deletedPathList[for_k].end()) {
            if (workloadindex&&labelidhash[(*itr_de).label]==0) {
            ...
            while(left<=right){
                ...
                if(label2history[for_l][for_h]==history_of_dpath){
                    ...
                    for(int i=0;i<dPath.labelset.size();i++){
                        if(labelidhash[dPath.labelset[i]]-1==for_l){sameLabelFlag=true;break;}
                    }
                    if(!sameLabelFlag){
...
void BisimulationIndex::AddEdge(Edge aedge, OriginalGraph& graph)
{
    ...
    for(auto aPath : addedPathWLabelset){
        ...
        if(for_h!=-1){
            history_of_apath=for_h;

            int newhistory = history2path.size();

            for(int i=0;i<aPath.labelset.size();i++){
                if(labelidhash[aPath.labelset[i]]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(newhistory);
                    label2history.push_back(temphistory);
                    labelidhash[aPath.labelset[i]]=label2history.size();
                }
                else {label2history[labelidhash[aPath.labelset[i]]-1].push_back(newhistory);}
            }
            for (int for_l = 0; for_l < label2history.size(); for_l++) {
                if(label2history[for_l].empty())continue;
                if(label2history[for_l][label2history[for_l].size() -1]<history_of_apath)continue;
                ...
                while(left<=right){
                    for_h=(left+right)/2;
                    if(label2history[for_l][for_h]==history_of_apath){

                        bool sameLabelFlag=false;
                        for(int i=0;i<aPath.labelset.size();i++){
                            if(labelidhash[aPath.labelset[i]]-1==for_l){sameLabelFlag=true;break;}
                        }
                    ...
                    else if(label2history[for_l][for_h]<history_of_apath){
                        left = for_h+1;
                    }
                    else {
                        right = for_h-1;
            ...
            for(auto for_label : aPath.labelset){
                 if(labelidhash[for_label]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(newhistory);
                    label2history.push_back(temphistory);
                    labelidhash[for_label]=label2history.size();
                }
                else {label2history[labelidhash[for_label]-1].push_back(newhistory);}
            }
...
void BisimulationIndex::DeleteWorkload(vector<int> labelsequences_, OriginalGraph& graph_) {
    long int labelid = encodeLabel(labelsequences_, labelnum, k);
    label2history[labelidhash[labelid]-1].clear();
    labelidhash[labelid] = 0;
    workload[labelid]=0;
}
...
void BisimulationIndex::AddWorkload(vector<int> labelsequence_, OriginalGraph& graph_) {
    ...
    long int labelid = encodeLabel(labelsequence_, labelnum, k);
    if(labelidhash[labelid]>0)return;
    if(workload[labelid]==1)return;
    workload[labelid]=1;
    //labelidhash[labelid]=true;
    ...
    long int label=labelsequence_[0];
    labelset.push_back(label);

    google::dense_hash_map<long int, int> labelidhash_addworkload;
    labelidhash_addworkload.set_empty_key(-1);
    ...
    for(int i=1;i<labelsequence_.size();i++){
        ...
        labelset.push_back(label);
        labelidhash_addworkload[label]=1;
    }
    ...
    clock_gettime(CLOCK_REALTIME, &h2p_update_endTime);
    maintainTime = (h2p_update_endTime.tv_sec - h2p_update_startTime.tv_sec) + (h2p_update_endTime.tv_nsec - h2p_update_startTime.tv_nsec) * pow(10, -9);
    cout<<"length 0 enumeration time: "<<maintainTime<<endl;
                    ...
                    if (pathid == 0) {
                        ...
                        for (auto label_l : segment.labelsequence) {
                            for (auto label_0 : seg[0][id_path0].labelsequence) {
                                long int edgelabel = (1 + label_l) * labelnum + label_0;
                                if (labelidhash_addworkload[edgelabel]>0)tempSegment.labelsequence.insert(edgelabel);
                            }
                        }
                        ...
                        for (auto label_l : segment.labelsequence) {
                            for (auto label_0 : seg[0][id_path0].labelsequence) {
                                long int edgelabel = (1 + label_l) * labelnum + label_0;
                                if (labelidhash_addworkload[edgelabel]>0)seg[for_k][pathid-1].labelsequence.insert(edgelabel);
                            }
                        }
                    }
                }
    ...
    clock_gettime(CLOCK_REALTIME, &h2p_update_endTime);
    maintainTime = (h2p_update_endTime.tv_sec - h2p_update_startTime.tv_sec) + (h2p_update_endTime.tv_nsec - h2p_update_startTime.tv_nsec) * pow(10, -9);
    cout<<"length k enumeration time: "<<maintainTime<<endl;
    ...
    for(int for_h=0;for_h < history2path.size();for_h++) { // storing all paths associated with the paths
    ...
    clock_gettime(CLOCK_REALTIME, &h2p_update_endTime);
    maintainTime = (h2p_update_endTime.tv_sec - h2p_update_startTime.tv_sec) + (h2p_update_endTime.tv_nsec - h2p_update_startTime.tv_nsec) * pow(10, -9);
    cout<<"history check time: "<<maintainTime<<endl;
...
//
        ...
        if(pathhistoryid[segcount]!= -1) {// if there is  same paths
            ...
            else {label2history[labelidhash[labelid]-1].push_back(newhistory);}
            ...
            if(labelidhash[labelid]==0){
                    vector<int> temphistory;
                    temphistory.clear();
                    temphistory.push_back(history2path.size());
                    label2history.push_back(temphistory);
                    labelidhash[labelid]=label2history.size();
                    indexhash_labelid[label2history.size()]=labelid;
                }
            else {label2history[labelidhash[labelid]-1].push_back(history2path.size());}
    ...
    cout<<"l2h update time"<<l2h_updateTime<<endl;
    cout<<"h2p update time"<<h2p_updateTime<<endl;


}

void BisimulationIndex::Reoptimization() {

    vector<pair<int,int>> pathInhistory;
    pathInhistory.resize(history2path.size());
    vector<vector<int>> equivalentCheckData;
    equivalentCheckData.resize(history2path.size());
    vector<int> equivalentHistory;
    equivalentHistory.resize(history2path.size());
    cout<<history2path.size()<<endl;
    for(int i=0;i<equivalentHistory.size();i++)equivalentHistory[i]=0;

    for(int for_h=0; for_h < pathInhistory.size();for_h++){
        if(!history2path[for_h].empty()){
            pathInhistory[for_h]=history2path[for_h][0];
            if(history2path[for_h][0].first==history2path[for_h][0].second)equivalentCheckData[for_h].push_back(1);
            else equivalentCheckData[for_h].push_back(0);
        }
    }

    //Search label sequences//
    for(int for_h=0;for_h<pathInhistory.size();for_h++) {
        for (int for_l = 0; for_l < label2history.size(); for_l++) {
            ...
            int historylocation;
            while(left<=right){
                historylocation=(left+right)/2;
                if(label2history[for_l][historylocation] == for_h){
                    equivalentCheckData[for_h].push_back(for_l);
                    break;
                }
                else if(label2history[for_l][historylocation] < for_h){
                    left = historylocation+1;
                }
                else {
                    right = historylocation-1;
                }
            }
    ...
    int numberOfhisotry=0;
    for(int for_h=0;for_h<equivalentHistory.size();for_h++) {

        if(equivalentHistory[for_h]==0){
            equivalentHistory[for_h]=for_h+1;
            numberOfhisotry++;
        }
        else continue;

        for(int for_hh=for_h+1;for_hh<equivalentCheckData.size();for_hh++){

            if(equivalentHistory[for_hh]!=0)continue;
            if(equivalentCheckData[for_h].size()!=equivalentCheckData[for_hh].size())continue;

            bool equivaleceFlag=true;
            for(int for_checkdata=0;for_checkdata<equivalentCheckData[for_hh].size();for_checkdata++){

                if(equivalentCheckData[for_h][for_checkdata]!=equivalentCheckData[for_hh][for_checkdata]){
                    equivaleceFlag=false;
                    break;
                }
            }
            if(equivaleceFlag)equivalentHistory[for_hh]=equivalentHistory[for_h];
        }
    }
    ...
    for(int for_h=0;for_h<history2path.size();for_h++){
        ...
        if(equivalentHistory[for_h]==for_h+1)continue;

        for (int for_count_h2p = 0; for_count_h2p < history2path[for_h].size(); for_count_h2p++) {

            history2path[equivalentHistory[for_h]-1].push_back(history2path[for_h][for_count_h2p]);

            string s = to_string(history2path[for_h][for_count_h2p].first)+"-"+to_string(history2path[for_h][for_count_h2p].second);
            path2history[s]=equivalentHistory[for_h];
        }

        history2path[for_h].clear();
    }

    for(int for_h=0;for_h<history2path.size();for_h++)sort(history2path[equivalentHistory[for_h]-1].begin(), history2path[equivalentHistory[for_h]-1].end(),paircmp);

    for(int for_h=0;for_h<equivalentHistory.size();for_h++){

        //int deleteHistory=for_h;
        if(for_h==equivalentHistory[for_h]-1)continue;
        ...
        for(int for_l=0;for_l<label2history.size();for_l++) {
            ...
            while(left<=right){
                historylocation=(left+right)/2;
                if(label2history[for_l][historylocation] == for_h){
                    label2history[for_l].erase(label2history[for_l].begin()+historylocation);
                    break;
                }
                else if(label2history[for_l][historylocation] < for_h){
                    left = historylocation+1;
...
void BisimulationIndex::CheckIndex(string filename){

    std::ofstream fout(filename, ios::app);

    vector<vector<int>> labelsInhisotry;
    labelsInhisotry.resize(history2path.size());

    for(int i=0; i<label2history.size();i++) {
        if(label2history[i].empty())continue;
        for(int j=0; j < label2history[i].size();j++){
            labelsInhisotry[label2history[i][j]].push_back(i);
        }
    }
   ...
   for(int i=0; i<history2path.size();i++) {
       ...
       for(int j=0; j< history2path[i].size();j++){
           fout<<history2path[i][j].first<<","<<history2path[i][j].second<<":"<<i<<": ";

           for(int k=0;k < labelsInhisotry[i].size();k++){
               fout<<labelsInhisotry[i][k]<<",";
           }
           fout<<endl;
       }
   }
...
void BisimulationIndex::Output(string indexfilename){
    ...
    string outputWorkload = "./index/workload_"+indexfilename;
    ...
    for(int i=0; i<label2history.size();i++) {
        ...
        if(label2history[i].empty())continue;
        fout_l2h<<indexhash_labelid[i+1]<<" ";
    ...
    if(workloadindex) {
        std::ofstream fout_workload(outputWorkload, ios::app);
        for (auto itr = workload.begin(); itr != workload.end(); ++itr) {
            if(itr->second==1) {
                vector<int> labelsequence = decodeLabel(itr->first, labelnum, k);
                if(labelsequence.size()==1)continue;
                for(auto label : labelsequence) {
                    fout_workload << label<<" ";
                }
                fout_workload<<endl;
            }
